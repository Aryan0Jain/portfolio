...
import { ROLES, ROLE_ORDER, METRICS, type RoleKey, type Metric } from '@/lib/data'
import { ArrowIcon, DownloadIcon } from './icons'

function useTypewriter(strings: string[], speed = 45, pause = 1700) {
...
export default function Hero({ activeRole }: HeroProps) {
  ...
  const lastSetRef = useRef(activeIdx)
  useEffect(() => {
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-7xl px-6">
        ...
        {/* Sub */}
        <p
          className="mt-4 max-w-2xl text-white/55 text-base md:text-lg"
...
function MetricChip({ m, accent, idx }: { m: Metric; accent: string; idx: number }) {
  return (
    <div
