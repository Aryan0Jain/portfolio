  ...
  return (
    <section id="contact" ref={ref} className="relative py-20">
      <div className="mx-auto max-w-7xl px-6">
        <window.SectionLabel num="06" title="Contact" accent={accent} />

        <div className="mt-6 grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
          <window.TerminalCard accent={accent} className="lg:col-span-7">
            <window.TerminalLine prompt accent={accent}>
            ...
            <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-3 font-mono text-[13px]">
              <ContactRow accent={accent} k="email" v="aryan.jain@example.com" href="mailto:aryan.jain@example.com" />
              <ContactRow accent={accent} k="github" v="github.com/aryanjain" href="https://github.com/" />
              <ContactRow accent={accent} k="linkedin" v="linkedin.com/in/aryanjain" href="https://linkedin.com/" />
        ...
        <footer className="mt-16 flex flex-wrap items-center justify-between gap-3 font-mono text-[11px] text-white/35 border-t border-white/8 pt-6">
          <div>© 2026 aryan.jain — built in React + Tailwind</div>
          <div>
            uptime: <span className="text-white/60">99.9%</span> · last_deploy: <span className="text-white/60">just now</span>
          </div>
        </footer>
      </div>
    </section>
...
function ContactRow({ k, v, href, accent }) {
  return (
    <a
      href={href}
    ...
    >
      ...
      <span className="text-white/85 group-hover:text-white truncate flex items-center gap-1.5" style={{ color: undefined }}>
        {v}
        <window.Icon.Arrow style={{ color: accent }} />
      </span>
    </a>
  );
}
